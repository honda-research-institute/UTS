...
def main():
    ...
    if cfg.isTrain or cfg.continue_train:
        ...
        with tf.device('/cpu:0'):
            if cfg.trimmed:
                train_data = FrameGeneratorTrimmed(cfg)
            else:
                train_data = FrameGenerator(cfg)
            train_data.print_config()
#            val_data = FrameGeneratorTrimmed(cfg, isTrain=False)
            val_data = FrameGenerator(cfg, isTrain=False)
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epoch_start, cfg.n_epochs+1):
                ...
                while True:
                    try:
                        x_batch, y_batch = sess.run(next_train)
...
#                        break
        ...
        with tf.device('/cpu:0'):
            if cfg.trimmed:
                val_data = FrameGeneratorTrimmed(model_cfg, isTrain=False)
            else:
                val_data = FrameGenerator(model_cfg, isTrain=False)
            val_data.print_config()
