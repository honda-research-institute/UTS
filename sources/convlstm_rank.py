...
from models.conv_lstm_rank import ConvLSTMRank
from utils.data_io import DataGeneratorSensorRank
...
def main():
    ...
    #        train_data = DataGeneratorSensorTrimmed(cfg)
            train_data = DataGeneratorSensorRank(cfg)
            train_data.print_config()
            val_data = DataGeneratorSensorRank(cfg, isTrain=False)
        ...
        # Train the model
        x1 = tf.placeholder(tf.float32, (cfg.batch_size, cfg.max_time, cfg.n_h, cfg.n_w,cfg.n_input))
        x2 = tf.placeholder(tf.float32, (cfg.batch_size, cfg.max_time, cfg.n_h, cfg.n_w,cfg.n_input))
        y = tf.placeholder(tf.int32, (cfg.batch_size, 1))   # adjusted shape for data loader
        seq_len1 = tf.placeholder(tf.int32, (cfg.batch_size,))
        seq_len2 = tf.placeholder(tf.int32, (cfg.batch_size,))
    
    
        # Train the model
        model = ConvLSTMRank(cfg)
        model.print_config()
        model.build_network(x1, x2, y, seq_len1, seq_len2)
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(epoch_start, cfg.n_epochs+1):
                ...
                while True:
                    try:
                        x_batch1, x_batch2, y_batch, len_batch1, len_batch2 = sess.run(next_train)
                ...
                while True:
                    try:
                        x_batch1, x_batch2, y_batch, len_batch1, len_batch2 = sess.run(next_val)
                        val_loss.append(sess.run(model.loss, feed_dict={
                                                                x1: x_batch1,
                                                                x2: x_batch2,
                                                                y: y_batch,
                                                                seq_len1: len_batch1,
        ...
        model = ConvLSTMRank(model_cfg)
        model.print_config()
        model.build_network(x1, x2, y, seq_len1, seq_len2)

        with tf.device('/cpu:0'):
    #        train_data = DataGeneratorSensorTrimmed(cfg)
            val_data = DataGeneratorSensorRank(model_cfg, isTrain=False)
