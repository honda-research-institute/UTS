...
from utils.data_io import DataGeneratorSensor, DataGeneratorSensorTrimmed
...
def main():
    ...
    elif cfg.model_type == "convbilstm":
        from models.conv_bilstm import ConvBiLSTM as Model
    elif cfg.model_type == "convtsn":
    ...
    #        train_data = DataGeneratorSensorTrimmed(cfg)
            train_data = DataGeneratorSensor(cfg)
            train_data.print_config()
            val_data = DataGeneratorSensor(cfg, isTrain=False)
            ...
            for epoch in range(epoch_start, cfg.n_epochs+1):
                ...
                while True:
                    try:
                        ...
                        if cfg.is_classify:
                            acc = accuracy_score(y_batch.argmax(axis=1), temp_pred)
                        else:
                            acc = 0
                ...
                if cfg.is_classify:
                    acc = accuracy_score(y_truth.argmax(axis=1), pred)
                else:
                    acc = 0
                print "\n"
        ...
        with tf.device('/cpu:0'):
    #        train_data = DataGeneratorSensorTrimmed(cfg)
            val_data = DataGeneratorSensor(model_cfg, isTrain=False)
