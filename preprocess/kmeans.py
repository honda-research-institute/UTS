...
#        label = np.sum(label, axis=1)
        ...
        # load features
        feat = h5py.File(os.path.join(cfg.sensor_root, session_id+'/feat_quan.h5'), 'r')['feats'][:]
#        feat_back.append(feat[label==0])    # using only background videos
    ...
    pkl.dump(kmeans, open(os.path.join(cfg.result_root, 'kmeans_featquan_%d.pkl' % K), 'w'))
    print ("Saved kmeans_featquan_%d.pkl" % K)
    ...
    for session_id in cfg.all_session:
        ...
        with h5py.File(os.path.join(cfg.sensor_root, session_id+'/feat_quancluster'+str(K)+'.h5'),'w') as fout:
